...
$userid = '1';
$idtypegallery = 'imgavatar';


$query = "SELECT *, @rank :=  @rank + 1 AS rank_av FROM gallery g left join post p on p.ID = g.postid, (SELECT  @rank := 0) r where membersid = '{$userid}' and gallerytype = '{$idtypegallery}' ORDER BY g.id DESC;";
...
<div class="col pd_5 md-width_50">
    <div class="position-relative height_100">
        <div data-bs-toggle="modal" data-bs-target="#myModal2">
            <img src="../images/<?php echo $row['url'] ?>" class="rounded-3 height_100" alt="" style="height: 250px;"
                data-bs-target="#carouselExampleCaptions2" data-bs-slide-to="<?php echo $row['rank_av'] - 1; ?>"
                aria-current="true" aria-label="Slide <?php echo $row['rank_av']; ?>">
        </div>
        ...
        <div class=" d-flex align-items-center position-absolute top-0 end-0 rounded-circle pd_5 button_edit mg_5">
            <div class="dropdown" style="height: 24px;">
                <div class=" dropdown-toggle button_dr_afet" type="button" data-bs-toggle="dropdown"
                    aria-expanded="false">
                ...
                <ul class="dropdown-menu z_index_10">
                    ...
                    <li>
                        <button class="btn d-flex align-items-center  justify-content-between delete_jqa delete_anh2"
                            id="<?php echo $row['postid'] ?>">
                            <span class="material-icons fz_12 text-black-50">
                                delete
                            </span>
